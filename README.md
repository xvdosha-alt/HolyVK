...
Браузерное расширение: в `vk.com/im` рядом с ФИО показывает игровой ник и должность модератора HolyWorld.
...
1. Content-скрипт на `vk.com/im` тянет staff через прокси `journal.dosha.pw` (HMAC + AES), кэширует в `storage`.
